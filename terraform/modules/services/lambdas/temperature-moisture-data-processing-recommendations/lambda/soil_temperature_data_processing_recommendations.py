...
AGRICULTURAL_TEMPERATURE_RECOMMENDATIONS_DYNAMODB_TABLE_NAME = 'AgriculturalTemperatureRecommendations'
TEMPERATURE_AVERAGES_DATA_TABLE_NAME = 'TemperatureAverages'
...
def get_latest_temperature():
    try:
        table = dynamodb.Table(TEMPERATURE_AVERAGES_DATA_TABLE_NAME)
...
def store_temperature_data(temperature, status, timestamp):
    ...
    try:
        ...
        if error_code == 'ResourceNotFoundException':
            print(f"A tabela {TEMPERATURE_AVERAGES_DATA_TABLE_NAME} não foi encontrada. Verifique o nome da tabela e a região.")
...
def store_recommendation(recommendations_dict, temperature, status, timestamp):
    try:
        ...
        print(f"Nome da tabela DynamoDB: {AGRICULTURAL_TEMPERATURE_RECOMMENDATIONS_DYNAMODB_TABLE_NAME}")
        
        table = dynamodb.Table(AGRICULTURAL_TEMPERATURE_RECOMMENDATIONS_DYNAMODB_TABLE_NAME)
        date = timestamp.split('T')[0]
        ...
        error_code = e.response['Error']['Code']
        if error_code == 'ResourceNotFoundException':
            print(f"A tabela {AGRICULTURAL_TEMPERATURE_RECOMMENDATIONS_DYNAMODB_TABLE_NAME} não foi encontrada. Verifique o nome da tabela e a região.")
        elif error_code == 'ConditionalCheckFailedException':
            print(f"Erro de condição ao atualizar o item para a data {date}. Verifique as condições de atualização.")
        else:
            print(f"Erro ao armazenar recomendações: {str(e)}")
