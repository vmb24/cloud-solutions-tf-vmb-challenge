...
air_temperature_averages_table = dynamodb.Table('AirTemperatureAverages')
...
def lambda_handler(event, context):
    ...
    try:
        if 'httpMethod' in event:
            ...
            logger.info("Coletando dados de temperatura do ar do banco de dados")
...
def handle_database_event():
    ...
    if temperature_data:
        ...
        logger.error("Não foi possível obter dados de temperatura do ar de nenhuma tabela")
        return create_response(404, "Dados de temperatura do ar não encontrados")

def get_latest_temperature_from_averages():
    try:
        logger.info("Obtendo a última temperatura do ar da tabela AirTemperatureAverages")
        response = air_temperature_averages_table.scan(Limit=1)
        ...
        logger.error(f"Erro ao obter temperatura do ar da tabela AirTemperatureAverages: {str(e)}")
        return None

def get_latest_temperature_from_history():
    try:
        logger.info("Obtendo a última temperatura do ar da tabela AirTemperatureHistory")
        response = air_temperature_history_table.scan(Limit=1)
        items = response.get('Items', [])
        if items:
            latest_item = items[0]
            return {
        ...
        logger.error(f"Erro ao obter temperatura do ar da tabela AirTemperatureHistory: {str(e)}")
        return None
...
def process_temperature_data(data):
    try:
        ...
        # Validando temperatura do ar
        if not validate_temperature_data(temperature):
            return create_response(400, 'Dados de temperatura do ar inválidos ou ausentes')

        # Gerando o plano de tarefas com a IA
        logger.info(f"Processando dados de temperatura do ar: temperature={temperature}, status={status}, crops={crops}")
        ...
        return create_response(500, f"Erro ao processar dados de temperatura do ar: {str(e)}")
...
def validate_temperature_data(temperature):
    ...
    try:
        ...
        if temperature_value < 0 or temperature_value > 100:
            logger.error(f"Valor de temperatura do ar fora do intervalo válido: {temperature_value}")
            return False
    except ValueError:
        logger.error(f"Valor de temperatura do ar não é um número válido: {temperature}")
        return False
...
def store_task_plan(plan, temperature, status):
    ...
    logger.info(f"[{timestamp}] Iniciando armazenamento do plano de tarefas de temperatura do ar")
    ...
    try:
        ...
        logger.info(f"[{timestamp}] Convertendo temperatura do ar para Decimal")
        ...
        response = air_temperature_task_plan_table.put_item(Item=task_item)
        ...
        logger.info(f"[{timestamp}] Plano de tarefas de temperatura do ar armazenado com sucesso. ID: {plan_id}")
        return plan_id
        ...
        logger.error(f"[{timestamp}] Erro inesperado ao armazenar plano de tarefas de temperatura do ar: {str(e)}")
        logger.exception("Detalhes do erro:")
        return None
    finally:
        logger.info(f"[{timestamp}] Finalizando operação de armazenamento do plano de tarefas de temperatura do ar")
