...
resource "aws_api_gateway_deployment" "soil_data_processing_recommendations_api_deployment" {
  depends_on = [
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_resource.moisture.id,
      ...
      aws_api_gateway_method.get_moisture.id,
      ...
      aws_api_gateway_integration.get_moisture_integration.id,
