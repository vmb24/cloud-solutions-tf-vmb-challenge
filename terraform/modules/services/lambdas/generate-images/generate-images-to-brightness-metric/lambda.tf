...
resource "aws_lambda_function" "generate_images_to_brightness_metric" {
  function_name    = "generate_gifs_to_brightness_metric"
  role             = var.generate_images_to_brightness_metric_lambda_role_arn
...
resource "aws_lambda_permission" "allow_s3_generate_images_to_brightness_metric" {
  statement_id  = "AllowS3Invoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.generate_images_to_brightness_metric.function_name
  principal     = "s3.amazonaws.com"
  source_arn    = var.brightness_media_bucket_arn
}