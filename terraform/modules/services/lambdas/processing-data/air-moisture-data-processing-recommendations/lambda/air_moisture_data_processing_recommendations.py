...
AGRICULTURAL_AIR_HUMIDITY_RECOMMENDATIONS_DYNAMODB_TABLE_NAME = 'AIAgriculturalAirHumidityRecommendations'
AIR_HUMIDITY_AVERAGES_DATA_TABLE_NAME = 'AirHumidityAverages'
...
def lambda_handler(event, context):
    ...
    elif 'humidity' in event:
        return process_humidity_data(event)
...
def handle_api_gateway_event(event):
    ...
    if http_method == 'GET':
        if path == '/humidity':
            return get_latest_humidity()
        elif path == '/humidity/history':
...
def get_latest_humidity():
    try:
        table = dynamodb.Table(AIR_HUMIDITY_AVERAGES_DATA_TABLE_NAME)
        ...
        if not items:
            return {
                'statusCode': 404,
                'body': json.dumps({'error': 'Nenhum dado de umidade do ar encontrado'}),
                'headers': get_cors_headers()
            }
        ...
        print(f"Erro ao obter a última leitura de umidade do ar: {str(e)}")
        return {
            'statusCode': 500,
            'body': json.dumps({'error': f"Ocorreu um erro ao obter a última leitura de umidade do ar: {str(e)}"}),
            'headers': get_cors_headers()
        }

def process_humidity_data(event):
    try:
        humidity = event['humidity']
        status = event['status']
        timestamp = datetime.now().isoformat()

        store_humidity_data(humidity, status, timestamp)

        recommendations_result = generate_all_recommendations(humidity, status)

        return recommendations_result
    except Exception as e:
        print(f"Erro ao processar dados de umidade do ar: {str(e)}")
        return {
            'statusCode': 500,
            'body': json.dumps({'error': f"Falha ao processar dados de umidade do ar: {str(e)}"}),
            'headers': get_cors_headers()
        }

def store_humidity_data(humidity, status, timestamp):
    table = dynamodb.Table(AIR_HUMIDITY_AVERAGES_DATA_TABLE_NAME)
    ...
    try:
        ...
        if response.get('Attributes', {}).get('readings', []):
            print(f"Dados de umidade do ar atualizados para a data {date}")
        else:
            print(f"Novo item criado para a data {date}")
        
        print(f"Dados de umidade do ar armazenados com sucesso para a data {date}")
    except ClientError as e:
        error_code = e.response['Error']['Code']
        if error_code == 'ResourceNotFoundException':
            print(f"A tabela {AIR_HUMIDITY_AVERAGES_DATA_TABLE_NAME} não foi encontrada. Verifique o nome da tabela e a região.")
        elif error_code == 'ConditionalCheckFailedException':
            print(f"Erro de condição ao atualizar o item para a data {date}. Verifique as condições de atualização.")
        else:
            print(f"Erro ao armazenar dados de umidade do ar: {str(e)}")
        raise
    except Exception as e:
        print(f"Erro inesperado ao armazenar dados de umidade do ar: {str(e)}")
        raise

def get_humidity_history(days=30):
    table = dynamodb.Table(AIR_HUMIDITY_AVERAGES_DATA_TABLE_NAME)
...
def generate_all_recommendations(humidity, status):
    timestamp = datetime.now().isoformat()
    
    print(f"Gerando recomendações para todos os tópicos. Umidade do ar: {humidity}%, Status: {status}")
    ...
    - Umidade atual do ar: {humidity}%
    - Status atual: {status}

    Forneça uma recomendação prática e acionável para cada um dos seguintes tópicos:
    {topics_prompt}

    Cada recomendação deve ser específica, detalhada e diretamente relacionada ao nível de umidade do ar atual. Responda em português.
    ...
    {{
        "Controle de Doenças": "Recomendação para Controle de Doenças...",
        "Manejo de Irrigação": "Recomendação para Manejo de Irrigação...",
        ...
    }}
    ...
    try:
        store_recommendation(recommendations_dict, humidity, status, timestamp)
...
def store_recommendation(recommendations_dict, humidity, status, timestamp):
    try:
        print("Tentando armazenar recomendações")
        print(f"Nome da tabela DynamoDB: {AGRICULTURAL_AIR_HUMIDITY_RECOMMENDATIONS_DYNAMODB_TABLE_NAME}")
        
        table = dynamodb.Table(AGRICULTURAL_AIR_HUMIDITY_RECOMMENDATIONS_DYNAMODB_TABLE_NAME)
        date = timestamp.split('T')[0]
        ...
        error_code = e.response['Error']['Code']
        if error_code == 'ResourceNotFoundException':
            print(f"A tabela {AGRICULTURAL_AIR_HUMIDITY_RECOMMENDATIONS_DYNAMODB_TABLE_NAME} não foi encontrada. Verifique o nome da tabela e a região.")
        elif error_code == 'ConditionalCheckFailedException':
            print(f"Erro de condição ao atualizar o item para a data {date}. Verifique as condições de atualização.")
        else:
            print(f"Erro ao armazenar recomendações: {str(e)}")
