...
def store_temperature_data(temperature, status, timestamp):
    ...
    try:
        ...
        if response.get('Attributes', {}).get('readings', []):
            print(f"Dados de temperatura do ar atualizados para a data {date}")
        else:
...
def generate_all_recommendations(temperature, status):
    ...
    print(f"Gerando recomendações para todos os tópicos. Temperatura do ar: {temperature}°C, Status: {status}")
    ...
    Cada recomendação deve ser específica, detalhada e diretamente relacionada ao nível de temperatura do ar atual. Responda em português.
    ...
    {{
        ...
        "Controle de Pragas e Doenças": "Recomendação para Controle de Pragas e Doenças...",
        ...
    }}
...
def store_recommendation(recommendations_dict, temperature, status, timestamp):
    try:
        ...
        error_code = e.response['Error']['Code']
        if error_code == 'ResourceNotFoundException':
            print(f"A tabela {AGRICULTURAL_AIR_TEMPERATURE_RECOMMENDATIONS_DYNAMODB_TABLE_NAME} não foi encontrada. Verifique o nome da tabela e a região.")
        elif error_code == 'ConditionalCheckFailedException':
            print(f"Erro de condição ao atualizar o item para a data {date}. Verifique as condições de atualização.")
        else:
            print(f"Erro ao armazenar recomendações: {str(e)}")
